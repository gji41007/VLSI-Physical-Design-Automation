...
public:
    ...
    std::vector<Component*> permutation(std::vector<Component*> candidates, int left, int right);
    int rearrangeDP(std::vector<Component*> candidates, int left, int right);
    bool insertComp(Component* comp, int x1, int x2);
    bool swapComp(Component* comp, int x1, int x2, int maxSize);
};
